...
function adaptNavigationStyleToScreenStyle(screen) {
  const navigationStyle = screen.navigationStyle;
  if (!navigationStyle) {
    return screen;
  }

  screen.styleParams = convertStyleParams(navigationStyle);

  return _.omit(screen, ['navigationStyle']);
}
...
function addNavigatorButtons(screen) {
  const Screen = getRegisteredScreen(screen.screen);
  if (screen.navigationButtons == null) {
    screen.navigationButtons = _.cloneDeep(Screen.navigationButtons);
  }
...
function getFab(screen) {
  let fab = screen.fab;
  if (screen.navigationButtons && screen.navigationButtons.fab) {
    fab = screen.navigationButtons.fab;
  }
...
function getLeftButton(screen) {
  ...
  if (screen.navigationButtons && screen.navigationButtons.leftButtons) {
    return screen.navigationButtons.leftButtons[0];
  }
...
function getLeftButtonDeprecated(screen) {
  if (screen.navigationButtons && screen.navigationButtons.leftButton) {
    return screen.navigationButtons.leftButton;
  }
...
function getRightButtons(screen) {
  if (screen.navigationButtons && screen.navigationButtons.rightButtons) {
    return screen.navigationButtons.rightButtons;
  ...
  if (Screen.navigationButtons && !_.isEmpty(Screen.navigationButtons.rightButtons)) {
    return _.cloneDeep(Screen.navigationButtons.rightButtons);
  }
...
function addNavigationStyleParams(screen) {
  const Screen = getRegisteredScreen(screen.screen);
  screen.navigationStyle = Object.assign({}, Screen.navigationStyle, screen.navigationStyle);
}
...
function showAlertToast(text) {
  ...
  adapted.backgroundColor = processColor("orange");
  adapted.actionColor = processColor('white');
  adapted.textColor = processColor('white');
